...
def MyModel(X_Data_Final, Y_Data_Final):
    ...
    X_tr1 = np.array(X_tr).repeat(3,axis=3)


    X_te = np.array(X_te).reshape(-1, SHAPE, SHAPE, 1)
    X_te1 = np.array(X_te).repeat(3,axis=3)
    ...
    print("形狀在此")
    print("X_tr.shape=", X_tr1.shape)
    print("X_te.shape=", X_te1.shape)



    model = InceptionResNetV2(include_top=False, weights='imagenet', input_tensor=Input(shape=(299, 299, 3)))
    x = model.output
